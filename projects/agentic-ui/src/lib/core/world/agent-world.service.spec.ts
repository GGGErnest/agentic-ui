...
function createService(withObserver = false): {
  ...
  TestBed.configureTestingModule({
    providers: [AgentWorldService, { provide: ApplicationRef, useValue: appRef }],
  });
...
describe('AgentWorldService', () => {
  ...
  describe('snapshot()', () => {
    ...
    it('should generate tool definitions for visible entries', () => {
      ...
      expect(snap.tools[0].function.name).toBe('btn-1__action__submit');
      expect(snap.tools[1].function.name).toBe('btn-1__action__cancel');
    ...
    it('should include multiple entries in context string', () => {
      const { service } = createService();
      service.register(
        createEntry({ id: 'a', role: 'Button', actions: [createMockAction('press', 'Press')] }),
      );
      service.register(
        createEntry({ id: 'b', role: 'Modal', actions: [createMockAction('close', 'Close')] }),
      );
  ...
  describe('executeAction()', () => {
    ...
    it('should pass parameters to action', async () => {
      ...
      service.register(
        createEntry({ id: 'field', actions: [{ name: 'type', description: 'Type', execute }] }),
      );
    ...
    it('should return error for non-existent action with available list', async () => {
      const { service } = createService();
      service.register(
        createEntry({ id: 'form', actions: [createMockAction('submit', 'Submit')] }),
      );
  ...
  describe('stability tracking', () => {
    ...
    it('resolves even when no further render fires after the call (#12)', async () => {
      const { service } = createService();
      // No renderComplete$.next() and no afterEveryRender in the test env: the
      // microtask fallback must still resolve so callers never hang.
      await expect(service.waitForStable()).resolves.toBeUndefined();
      expect(service.isStable()).toBe(false);
    });
  });
  ...
  describe('approval gate', () => {
    it('should call approval service when action.requiresApproval is true', async () => {
      ...
      expect(spy).toHaveBeenCalledWith('delete-btn', 'Button', 'delete', 'Delete item', {
        id: 'x',
      });
    });
  ...
  describe('tool name length validation', () => {
    it('should reject tool names exceeding 64 characters at registration', () => {
      const { service } = createService();
      const longId = 'a'.repeat(50); // 50 chars
      const longAction = 'b'.repeat(20); // 20 chars, total 80 chars with '__action__'
      expect(() =>
        service.register({
          id: longId,
          role: 'Button',
    ...
    it('should accept tool names at exactly 64 characters', () => {
      const { service } = createService();
      const longId = 'a'.repeat(47);
      const correctAction = 'b'.repeat(7); // 47 + '__action__' (10) + 7 = 64
      expect(() =>
        service.register({
          id: longId,
          role: 'Button',
  ...
  describe('snapshot budget control', () => {
    ...
    it('should cap tools when maxTools is set', () => {
      ...
      service['visibleIds'].set(
        new Set(['e-0', 'e-1', 'e-2', 'e-3', 'e-4', 'e-5', 'e-6', 'e-7', 'e-8', 'e-9']),
      );
    ...
    it('includes an entry all-or-nothing when maxTools falls mid-entry (#3)', () => {
      const { service } = createService();
      // First entry contributes 1 tool, second contributes 2 tools.
      service.register(createEntry({ id: 'solo', actions: [createMockAction('a1', 'one')] }));
      service.register(
        createEntry({
          id: 'pair',
          actions: [createMockAction('b1', 'one'), createMockAction('b2', 'two')],
        }),
      );
      service['visibleIds'].set(new Set(['solo', 'pair']));

      // Budget of 2: 'solo' (1 tool) fits; 'pair' (2 tools) would overflow -> dropped whole.
      const snap = service.snapshot({ maxTools: 2 });
      const names = snap.tools.map((t) => t.function.name);
      expect(names).toContain('solo__action__a1');
      expect(names).not.toContain('pair__action__b1'); // never partially represented
      expect(names).not.toContain('pair__action__b2');
    });

    it('omits dropped entries from the context string entirely (#4)', () => {
      const { service } = createService();
      service.register(createEntry({ id: 'solo', actions: [createMockAction('a1', 'one')] }));
      service.register(
        createEntry({
      ...
      const snap = service.snapshot({ maxTools: 2 });
      expect(snap.context).toContain('[solo]');
      // Dropped entry must not leak its id or actions into context.
      expect(snap.context).not.toContain('[pair]');
      expect(snap.context).not.toContain('b1');
      expect(snap.context).not.toContain('b2');
    });

    it('omits occluded entries from the context string (#4)', () => {
      const { service } = createService();
      ...
      service.register(createEntry({ id: 'modal', role: 'Modal', element: modalEl }));
      service.register(createEntry({ id: 'behind', element: behindEl }));
      service['visibleIds'].set(new Set(['modal', 'behind']));

      const snap = service.snapshot();
      expect(snap.context).toContain('[modal]');
      expect(snap.context).not.toContain('[behind]');
    });
  });
  ...
  describe('Input Schema', () => {
    it('should build tools from inputSchema when present', () => {
      ...
      expect(
        (tool.function.parameters.properties as Record<string, unknown>)['email'],
      ).toBeDefined();
    ...
    it('should fall back to parameters when inputSchema not present', () => {
      ...
      const action: AgentAction = {
        ...
        parameters: [{ name: 'count', type: 'number', description: 'Click count', required: true }],
        execute: vi.fn(),
      };
      ...
      const tool = snap.tools[0];
      expect(
        (tool.function.parameters.properties as Record<string, unknown>)['count'],
      ).toBeDefined();
    });
    ...
    it('should preserve nested object schemas in tools', () => {
      ...
      const snap = service.snapshot();
      const tool = snap.tools[0];
      const dataProps = (tool.function.parameters.properties as Record<string, any>)[
        'data'
    ...
    it('should preserve array item schemas in tools', () => {
      ...
      const snap = service.snapshot();
      const tool = snap.tools[0];
      const itemsProps = (tool.function.parameters.properties as Record<string, any>)[
        'items'
      ] as Record<string, any>;
  ...
  describe('Readable setter tools', () => {
    it('should generate setter tools for writable readables', () => {
      ...
      const setterTool = snap.tools.find((t) => t.function.name === 'comp1__write__config');
    ...
    it('should wrap readable value in "value" property', () => {
      ...
      const snap = service.snapshot();
      const setterTool = snap.tools.find((t) => t.function.name === 'comp1__write__count');
      expect(
        (setterTool?.function.parameters.properties as Record<string, unknown>)['value'],
      ).toBeDefined();
      expect(setterTool?.function.parameters.required).toContain('value');
    });
  });

  describe('snapshotAsync() visibility scoping (#2)', () => {
    it('only reads readables for visible, unoccluded entries', async () => {
      const { service } = createService();
      const visibleReadable = createMockReadable('visibleState', 'Visible state');
      const hiddenReadable = createMockReadable('hiddenState', 'Hidden state');

      service.register(createEntry({ id: 'on-screen', actions: [], readables: [visibleReadable] }));
      service.register(createEntry({ id: 'off-screen', actions: [], readables: [hiddenReadable] }));
      // Only 'on-screen' is in the viewport.
      service['visibleIds'].set(new Set(['on-screen']));

      const snap = await service.snapshotAsync();

      expect(snap.context).toContain('visibleState');
      expect(snap.context).not.toContain('hiddenState');
      // Off-screen readable must never be read.
      expect(hiddenReadable.read).not.toHaveBeenCalled();
      expect(visibleReadable.read).toHaveBeenCalled();
    });
  });
});
