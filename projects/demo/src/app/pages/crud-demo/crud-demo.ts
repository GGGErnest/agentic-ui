...
@Component({
    ...
    <div class="demo-container">
      <header class="demo-header">
        ...
        <p class="demo-subtitle">
          This page is instrumented for AI agents. Every button and the data table
          expose their capabilities through the World Registry — no DOM scraping needed.
        </p>
      </header>
  ...
  styles: [`
    ...
    .demo-header h1 { font-size: 24px; margin-bottom: 4px; }
    .demo-subtitle { color: #8b949e; font-size: 14px; margin-bottom: 20px; }
    .demo-toolbar {
      display: flex;
      gap: 10px;
      align-items: center;
    ...
    .demo-stats { margin-left: auto; color: #8b949e; font-size: 13px; }
    .demo-filters { display: flex; gap: 8px; margin-bottom: 16px; }
    .demo-footer {
      margin-top: 24px;
      padding: 12px;
      border-top: 1px solid #30363d;
    }
    .demo-hint { color: #8b949e; font-size: 13px; }
    ...
    .btn:hover { background: #1c2530; }
    .btn-primary { background: #238636; border-color: #238636; color: #fff; }
    .btn-primary:hover { background: #2ea043; }
    .btn-danger { color: #f85149; }
    .btn-danger:hover { background: #490202; border-color: #f85149; }
    .btn-secondary { color: #8b949e; }
    ...
    .modal label { font-size: 12px; color: #8b949e; text-transform: uppercase; }
...
export class CrudDemo {
  ...
  readonly tasks = signal<Task[]>([
    ...
    { id: '5', title: 'Add unit tests', priority: 'medium', status: 'in-progress', assignee: 'Diana' },
  ]);
  ...
  readonly filters = ['high', 'medium', 'low'].map(p => ({
    ...
    actions: [{
      ...
      execute: async () => {
        ...
        return { success: true, message: this.activeFilter() ? `Filtered by ${p} priority.` : 'Filter cleared.' };
      },
    }],
  }));
  ...
  save(): void {
    ...
    if (this.editId()) {
      this.tasks.update(list => list.map(t =>
        t.id === this.editId()
          ? { ...t, title: this.formTitle, priority: this.formPriority, assignee: this.formAssignee }
          : t
      ));
    } else {
      const newTask: Task = {
